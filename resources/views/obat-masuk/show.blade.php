...
<div class="bg-white rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.02)] border border-[#E6E4DD]/50 p-8 max-w-xl">
    ...
    <div class="space-y-6 mb-8">
        ...
        <div class="border-b border-[#E6E4DD]/60 pb-5">
            ...
            <p class="text-lg font-serif text-[#2D2A26]">{{ $obatMasuk->obat->nama_obat ?? '-' }}</p>
        </div>
        
        <div class="grid grid-cols-2 gap-6 border-b border-[#E6E4DD]/60 pb-5">
            <div>
                <p class="text-sm font-medium text-[#8F8C87] mb-1">Kategori</p>
                <p class="text-[15px] text-[#2D2A26]">{{ $obatMasuk->obat->kategori->nama_kategori ?? '-' }}</p>
            </div>
            <div>
                <p class="text-sm font-medium text-[#8F8C87] mb-1">Tanggal Masuk</p>
                <p class="text-[15px] text-[#2D2A26]">{{ \Carbon\Carbon::parse($obatMasuk->tanggal)->locale('id')->translatedFormat('d F Y') }}</p>
            </div>
        </div>

        <div class="border-b border-[#E6E4DD]/60 pb-5">
            <p class="text-sm font-medium text-[#8F8C87] mb-1">Kuantitas Masuk</p>
            <p class="text-[15px] font-medium text-[#2E5C3A] bg-[#F0F5F1] inline-flex px-3 py-1 rounded-lg">
                +{{ $obatMasuk->jumlah }} Unit
            </p>
        </div>
        ...
        <div>
            ...
            <p class="text-[15px] text-[#5C5954] leading-relaxed">
                {{ $obatMasuk->keterangan ?: 'Tidak ada catatan yang dilampirkan pada transaksi ini.' }}
            </p>
        </div>
