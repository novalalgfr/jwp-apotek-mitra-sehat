...
@section('header', 'Edit Obat Masuk')

@section('content')
<div class="bg-white rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.02)] border border-[#E6E4DD]/50 p-8 max-w-xl">

    <form method="POST" action="{{ route('obat-masuk.update', $obatMasuk) }}" class="space-y-5">
        @csrf @method('PUT')
            ...
            <select name="obat_id"
                ...
                @foreach($obats as $obat)
                    <option value="{{ $obat->id }}"
                        {{ old('obat_id', $obatMasuk->obat_id) == $obat->id ? 'selected' : '' }}>
        ...
        <div class="space-y-1.5">
            ...
            <input type="date" name="tanggal"
                   value="{{ old('tanggal', $obatMasuk->tanggal) }}"
                   class="w-full bg-[#F3F2EE] border border-transparent rounded-xl px-4 py-3 text-sm focus:bg-white focus:border-[#D6D3CD] focus:outline-none focus:ring-4 focus:ring-[#F3F2EE] transition-all">
        </div>
        ...
        <div class="space-y-1.5">
            ...
            <input type="number" name="jumlah" min="1"
                   value="{{ old('jumlah', $obatMasuk->jumlah) }}"
                   class="w-full bg-[#F3F2EE] border border-transparent rounded-xl px-4 py-3 text-sm focus:bg-white focus:border-[#D6D3CD] focus:outline-none focus:ring-4 focus:ring-[#F3F2EE] transition-all">
        </div>
        ...
        <div class="space-y-1.5 pb-2">
            ...
            <textarea name="keterangan" rows="3"
                      class="w-full bg-[#F3F2EE] border border-transparent rounded-xl px-4 py-3 text-sm focus:bg-white focus:border-[#D6D3CD] focus:outline-none focus:ring-4 focus:ring-[#F3F2EE] transition-all">{{ old('keterangan', $obatMasuk->keterangan) }}</textarea>
        </div>
