...
<div class="bg-white rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.02)] border border-[#E6E4DD]/50 p-8 max-w-xl">
    ...
    <form method="POST" action="{{ route('users.update', $user) }}" class="space-y-5">
        ...
        <div class="space-y-1.5">
            ...
            <select name="role"
                    class="w-full bg-[#F3F2EE] border border-transparent rounded-xl px-4 py-3 text-sm focus:bg-white focus:border-[#D6D3CD] focus:outline-none focus:ring-4 focus:ring-[#F3F2EE] transition-all">
                <option value="admin" {{ old('role', $user->role) == 'admin' ? 'selected' : '' }}>Admin (Standar)</option>
        ...
        <div class="space-y-1.5 pb-2">
            <label class="block text-sm font-medium text-[#5C5954]">Password Baru</label>
            <div class="relative">
                <input type="password" name="password" id="password-input"
                       class="w-full bg-[#F3F2EE] border border-transparent rounded-xl px-4 py-3 pr-11 text-sm focus:bg-white focus:border-[#D6D3CD] focus:outline-none focus:ring-4 focus:ring-[#F3F2EE] transition-all placeholder:text-[#A8A5A0]"
