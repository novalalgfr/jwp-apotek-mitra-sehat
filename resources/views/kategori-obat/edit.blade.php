    ...
    <form method="POST" action="{{ route('kategori-obat.update', $kategoriObat) }}" class="space-y-5">
        ...
        <div class="space-y-1.5 pb-2">
            ...
            <textarea name="deskripsi" rows="3"
                      class="w-full bg-[#F3F2EE] border border-transparent rounded-xl px-4 py-3 text-sm focus:bg-white focus:border-[#D6D3CD] focus:outline-none focus:ring-4 focus:ring-[#F3F2EE] transition-all">{{ old('deskripsi', $kategoriObat->deskripsi) }}</textarea>
        </div>
