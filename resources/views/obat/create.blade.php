...
<div class="bg-white rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.02)] border border-[#E6E4DD]/50 p-8 max-w-xl">
    ...
    <form method="POST" action="{{ route('obat.store') }}" class="space-y-5" enctype="multipart/form-data">
        @csrf
        ...
        <div class="space-y-1.5">
            ...
            <p class="text-[11px] text-[#8F8C87] mt-1">Format: JPG, PNG, WEBP. Maks 2MB.</p>
            @error('gambar')
                <p class="text-[#B3412F] text-xs mt-1 font-medium">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="space-y-1.5 pb-2">
            <label class="block text-sm font-medium text-[#5C5954]">Stok Awal <span class="text-[#B3412F]">*</span></label>
